...
export default function leftAlignedFlextree(options: any = {}): LayoutFunction {
  ...
  function getFlexNode() {
    ...
    return class FlexNode extends hierarchy.prototype.constructor {
      ...
      get xTopExtent() {
        return (this as any).data?.__layoutVerticalExtents?.top ?? this.xSize / 2;
      }
      get xBottomExtent() {
        return (this as any).data?.__layoutVerticalExtents?.bottom ?? this.xSize / 2;
      }
      get left() { return (this as any).x - this.xTopExtent; }
      get right() { return (this as any).x + this.xBottomExtent; }
      get root() {
  ...
  const layoutChildren = (w: any, y = 0) => {
    ...
    shiftChange(w);
    positionRoot(w); // 수정된 positionRoot 사용
    return w;
  };
  ...
  const separate = (w: any, i: any, lows: any) => {
    ...
    while (rContour && lContour) {
      ...
      const dist =
        ...
        rContour.xBottomExtent + lContour.xTopExtent +
